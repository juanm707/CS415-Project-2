...
int main() {
    ...
    //for (int i = 1; i < 2; i++) { to test 1*1 1*2 ... 999*999 worked for every case

    //std::vector<int> mainAnswer = karatsuba_multi(convertToVector2(1000), convertToVector2(1000));

    for (int j = 1; j < 2; j++) { //1000 ^ 3 = 1 000 000 000
        ...
        //mainAnswer = karatsuba_multi(mainAnswer, answer);
        ...
        /*if (check != (a * b)) {
            ...
            //std::cout << number << std::endl;
            printVector(answer);
            std::cout << "Using regular = " << check << std::endl << std::endl; //switched i = a, j = b
        }*/
...
std::vector<int> karatsuba_multi(std::vector<int> A, std::vector<int> B) { //assume even, and 4 or 2 digits so 1000*1000 or 23*14 = 322, 0486 * 0236 = 114696
    ...
    if ((numDigitsA == 1) && (numDigitsB == 1)) { //if both A and B are single digits
        ...
        std::vector <int> c2PLUSc1 = addVectors(c2,c1);
        std::vector <int> answer = addVectors(c2PLUSc1, c0);
    ...
    while (i < A.size() + 1) {
        ...
        if ((i == A.size())) {
            ...
            if (carryFlag == 1) {
                ...
                answer.push_back(num % 10); //FIX THIS CHECK THIS not single digit
...
std::vector<int> subtractVectors(std::vector<int> A, std::vector<int> B) {
    ...
    while (i < A.size()) {
        ...
        if (A[i] < B[i]) {
            ...
            //number = subtractVectors(convertToVector2(10), convertToVector2(B[i] - A[i]));

            if (carryFlag == 1) {
                num -= 1;
                //number = subtractVectors(number, convertToVector2(1));
            }
            carryFlag = 1;
